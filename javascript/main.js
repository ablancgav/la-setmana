document.addEventListener('DOMContentLoaded', () => {
    ...
    if (botoMenu && navContent) {
        ...
        document.addEventListener('click', (e) => {
            // comprovem si el menú està obert, si el clic és fora del menú i fora del botó
            if (navContent.classList.contains('show') && 
                !navContent.contains(e.target) && 
                !botoMenu.contains(e.target)) {
                tancaMenu();
            }
        });
    }
    ...
    /* ---- 12. animació d'aparició al fer scroll (banners) ---- */
    const banners = document.querySelectorAll('.banner__content');

    if (banners.length > 0) {
        const observerOptions = {
            root: null,
            rootMargin: '0px 0px -15% 0px', /* s'activa quan l'element ha entrat un 15% per la part inferior */
            threshold: 0
        };

        const observerBanners = new IntersectionObserver((entries, observer) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    /* afegim la classe que dispara el css */
                    entry.target.classList.add('is-visible');
                    
                    /* deixem d'observar-lo perquè l'animació només es reprodueixi un cop */
                    observer.unobserve(entry.target);
                }
            });
        }, observerOptions);

        banners.forEach(banner => observerBanners.observe(banner));
    }
});